...
startDevModeChaincodeProcess() {
  ...
  local CHAINCODE_DIR="$4"
  local ORG_MSP="$5"
  local PEER_TLS_CA_REL="$6"
  local ORDERER_TLS_CA_REL="$7"
  local TLS_ENABLED="$8"
  local PEER_ADDRESS="$9"
  local ORG_DOMAIN=${10}

  local CC_ID="${CHAINCODE_NAME}:${CHAINCODE_VERSION}"
  local CONTAINER_NAME="devmode-${CHAINCODE_NAME}"

  local BASE_PATH="$FABLO_NETWORK_ROOT/fabric-config/crypto-config"
  local PEER_TLS_CA=$(realpath "$BASE_PATH/$PEER_TLS_CA_REL")
  local ORDERER_TLS_CA=$(realpath "$BASE_PATH/$ORDERER_TLS_CA_REL")
  local CONFIG_PATH="$FABLO_NETWORK_ROOT/fabric-config/crypto-config/"
  # Generate certs for dev mode container
  echo "Generating certifcates for dev mode container"
  certsGenerateCCaaS "$CONFIG_PATH" "$CONTAINER_NAME" "$ORG_DOMAIN" "$CHAINCODE_NAME" "$PEER_ADDRESS"

  echo "Launching devmode chaincode container $CONTAINER_NAME"
  echo "   CHAINCODE: $CHAINCODE_NAME v$CHAINCODE_VERSION ($CHAINCODE_LANG)"
  echo "   MSP: $ORG_MSP"
  echo "   PEER_TLS_CA: $PEER_TLS_CA_REL"
  echo "   ORDERER_TLS_CA: $ORDERER_TLS_CA_REL"

  local TLS_PATH="$CONFIG_PATH/ccaas/$CONTAINER_NAME/tls"

  # Pick base image depending on lang
  local CC_IMAGE=""
  case "$CHAINCODE_LANG" in
    node)
      CC_IMAGE="node:16"
      START_CMD="cd /usr/src/app && npm install && npm run start:watch"
      ;;
    golang|go)
      CC_IMAGE="golang:1.20"
      START_CMD="cd /usr/src/app && go mod tidy && go run ."
      ;;
    java)
      CC_IMAGE="openjdk:11"
      START_CMD="cd /usr/src/app && ./gradlew installDist && ./build/install/chaincode/bin/chaincode"
      ;;
    *)
  ...
  DOCKER_NETWORK=$(docker inspect peer0.org1.example.com | jq -r '.[0].NetworkSettings.Networks | keys[0]')
