...
certsGenerate() {
  ...
  docker run -i -d -w="/" --name $CONTAINER_NAME fablo/fabric-tools:"${FABRIC_TOOLS_VERSION}" bash || removeContainer $CONTAINER_NAME
...
createChannelTx() {
  ...
  docker run --rm \
    ...
    fablo/fabric-tools:"${FABRIC_TOOLS_VERSION}" \
